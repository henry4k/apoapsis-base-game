...
vec3 CalcLightColor( vec3 albedo, vec3 specular, vec3 normalTS )
{
    vec3 lightDirectionTS  = normalize(LightDirectionTS);
    vec3 cameraDirectionTS = normalize(CameraDirectionTS);

    float lamberFactor = max(dot(lightDirectionTS, normalTS), 0.0);

    vec3 reflectionTS = reflect(lightDirectionTS, normalTS);
